...
function Filters.propagate!(m::SimpleStateTransition, xn, x; kwargs...)
    @. xn = x + m.b + randn() * m.P
    nothing
end
...
function Filters.likelihood(m::SimpleLikelyhoodModel, x, z; kwargs...)
...
pf = BootstrapParticleFilter{T}(
    ParticleState(p0, w0),
    SimpleStateTransition(Σw, ω),
    SimpleLikelyhoodModel(Σv),
    Resampling(SystematicResamplingAlgorithm(), EffectiveSamplesPolicy(300))
)
