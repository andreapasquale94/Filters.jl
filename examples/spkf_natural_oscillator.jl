...
spkf = SigmaPointsKalmanFilter{T}(
    SigmaPointKalmanState(x̄0, P0),
    SigmaPointsKalmanFilterPrediction{T}(
        work.model.state,
        work.model.process_noise,
        CDKFSigmaPoints(sqrt(3)),
        work.nx
    ),
    SigmaPointsKalmanFilterUpdate{T}(
        work.model.obs,
...
# Run the filter
estimates = run(SigmaPointKalmanState{T}, spkf, work.z_sim);
