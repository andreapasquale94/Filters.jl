...
"""
function cholesky_downdate!(L::LowerTriangular{T}, U::Matrix{T}; 
    buffer::Vector{T}=zeros(T, size(U, 1))) where {T <: AbstractFloat}
    n, k = size(U)
...
Modifies `L` in-place.
"""
function cholesky_update!(L::LowerTriangular{T}, U::Matrix{T}; 
    buffer::Vector{T}=zeros(T, size(U, 1))) where {T <: AbstractFloat}
    n, k = size(U)
    @assert size(L, 1) == n && size(L, 2) == n
    for j in 1:k
